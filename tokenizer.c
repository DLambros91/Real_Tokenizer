...
{	// Begin TKDestroy
	...
	// Free the memory allocated for tokenDesc
	free(tk->tokenDesc);
...
{ // Begin TKGetNextToken
	...
	*/
	if((char)tk->str[tk->curChar] == '\'')
	{
		...
		tk->curChar++;
		tk->tokenDesc[tk->manyTokens] = "string";
		break;
	}
	...
	else if((char)tk->str[tk->curChar] == '^')
	{
			arr[i] = tk->str[tk->curChar];
			i++;
			tk->tokenDesc[tk->manyTokens] = "Binary Operator: Bitwise Exclusive OR";
			tk->curChar++;
			if((char)tk->str[tk->curChar] == '=')
			{
				arr[i] = tk->str[tk->curChar];
	...
	else if((char)tk->str[tk->curChar] == '\%')
	{
		...
		else if((char)tk->str[tk->curChar] == 'd')
		{
			arr[i] = tk->str[tk->curChar];
			i++;
